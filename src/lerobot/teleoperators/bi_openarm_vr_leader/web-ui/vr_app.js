    ...
    if (this.leftHand && this.leftHand.object3D) {
        ...
        if (this.leftHand && this.leftHand.components && this.leftHand.components['tracked-controls']) {
            ...
            if (leftGamepad) {
                ...
                // 获取trigger值（通常是按钮0）
                const leftTriggerValue = leftGamepad.buttons[0]?.value || 0;
                leftController.trigger = leftTriggerValue; // 使用实际的trigger值而不是布尔值
                // 侧边按钮
                leftController.buttons = {
                    L_thumbstick: !!leftGamepad.buttons[3]?.pressed,  // 左手柄中键
    ...
    if (this.rightHand && this.rightHand.object3D) {
        ...
        if (this.rightHand && this.rightHand.components && this.rightHand.components['tracked-controls']) {
            ...
            if (rightGamepad) {
                ...
                // 获取trigger值（通常是按钮0）
                const rightTriggerValue = rightGamepad.buttons[0]?.value || 0;
                rightController.trigger = rightTriggerValue; // 使用实际的trigger值而不是布尔值
                // 侧边按钮
                rightController.buttons = {
                    R_thumbstick: !!rightGamepad.buttons[3]?.pressed,  // 右手柄中键
